...
define('BD_NOMBRE', 'bares_locales');
define('BD_USUARIO', 'bares_daniel');
define('BD_PASSWORD', '14093487');
...
$app->get('/locals', function() use($db) {
            ...
            $consulta = $db->prepare("select * from locals");
            $consulta->execute();
...
// El valor del parámetro :rID se pasará a la función de callback como argumento
$app->get('/locals/:rID', function($rID) use($db) {
            // Va a devolver un objeto JSON con los datos de usuarios.
            // Preparamos la consulta a la tabla.
            // En PDO los parámetros para las consultas se pasan con :nombreparametro (casualmente 
            // coincide con el método usado por Slim).
            // No confundir con el parámetro :rID que si queremos usarlo tendríamos 
            // que hacerlo con la variable $rID
            $consulta = $db->prepare("select * from locals where rID=:param1");
 
            // En el execute es dónde asociamos el :param1 con el valor que le toque.
            $consulta->execute(array(':param1' => $rID));
